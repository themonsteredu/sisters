...
- 알림 목록 화면과 읽음 처리 *(알림은 적재되지만 앱에서 보는 화면이 아직 없음. 헤더 벨은 미읽음 여부만 표시)*
- 말하기·듣기 문항의 실제 녹음과 음성 인식 *(`transcribeSpeech`는 구현되어 있으나 호출하는 곳이 없음. 현재 말하기 문항은 텍스트 입력으로 응시)*
- 서술형 문항의 부모 채점 화면 *(응시 기록에 `needs_parent_review`로 표시되고 알림도 가지만, 점수를 매기는 화면이 아직 없음)*
- 알림톡 발송 *(`sisters_profiles.phone`을 쓰는 화면이 없어 수신자 번호가 비어 있음)*
- 가족 정지(`suspended`) 적용 *(관리자 화면에 표시만 되고 접근을 막지 않음)*
