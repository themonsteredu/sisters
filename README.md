...
- **한 줄로 계획 만들기**: `민서 영어 문법 12강 8/10~8/28 평일 하루 2강, 8/15 빼고` 처럼 적으면 학생·과목·기간·학습량·요일·휴일·하루 최대량을 규칙 기반으로 뽑아 폼에 채워 줍니다. AI 키 없이 즉시 동작하고, 만들기 전에 부모가 확인합니다.
...
단위테스트는 목차 파싱, 휴일/요일/마감 충돌을 반영한 일정 배분, 결정적 채점(부분 응답·순서 문항·대체 정답 포함), 자연어 계획 해석, 계획 과제 생성, 제출 첨부 경로 규칙, API 키 암복호화, 동일 출처 검사, 레이트 리밋을 검증합니다.
