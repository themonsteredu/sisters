...
type LoginTab = "parent" | "student" | "admin";
...
export function LoginPanel({
  ...
  const [adminPin, setAdminPin] = useState(demo ? "123456" : "");
  ...
  function submit(event: FormEvent) {
    ...
    else if (tab === "student") requestLogin({ handle, pin });
    else requestLogin({ pin: adminPin });
  }
  ...
  return (
    <div className="w-full max-w-md rounded-[28px] border border-white/70 bg-white/95 p-6 shadow-2xl shadow-violet-900/10 backdrop-blur md:p-8">
      <div className="mb-6 grid grid-cols-3 rounded-xl bg-slate-100 p-1">
        {(["parent", "student", "admin"] as const).map((item) => (
          <button key={item} onClick={() => { setTab(item); setMessage(""); }} className={`rounded-lg px-3 py-2.5 text-sm font-bold transition ${tab === item ? "bg-white text-violet-700 shadow-sm" : "text-slate-500"}`}>
            {item === "parent" ? "부모" : item === "student" ? "학생" : "관리자"}
          </button>
        ))}
      </div>
      <div className="mb-6">
        <h1 className="text-2xl font-black tracking-tight">
          {tab === "parent" ? "부모님, 반가워요" : tab === "student" ? "오늘 공부를 시작해볼까요?" : "관리자 로그인"}
        </h1>
        <p className="mt-2 text-sm leading-6 text-slate-500">
          {tab === "parent" ? "이메일과 비밀번호로 바로 로그인하세요." : tab === "student" ? "부모님이 만들어 준 아이디와 6자리 PIN을 입력하세요." : "설정해 둔 관리자 PIN 6자리를 입력하세요."}
        </p>
      </div>
      <form onSubmit={submit} className="space-y-4">
        {tab === "parent" ? (
          ...
          <>
            <label className="block"><span className="mb-2 block text-sm font-bold text-slate-700">관리자 PIN</span><span className="flex items-center gap-2 rounded-xl border border-slate-200 px-3 focus-within:border-violet-400 focus-within:ring-4 focus-within:ring-violet-50"><ShieldCheck size={18} className="text-slate-400" /><input required type="password" inputMode="numeric" pattern="[0-9]{6}" minLength={6} maxLength={6} autoComplete="current-password" value={adminPin} onChange={(event) => setAdminPin(event.target.value.replace(/\D/g, ""))} className="h-12 min-w-0 flex-1 tracking-[.45em] outline-none" /></span></label>
            <p className="text-xs leading-5 text-slate-400">처음이라면 부모로 로그인한 뒤 <Link href="/settings/admin-pin" className="font-bold text-violet-700 hover:underline">설정에서 관리자 PIN 만들기</Link>를 눌러 주세요.</p>
          </>
        )}
        {message && <p className="rounded-xl bg-violet-50 p-3 text-sm font-medium text-violet-700" role="status">{message}</p>}
        <Button type="submit" className="h-12 w-full" disabled={loading}>{loading ? "확인 중…" : tab === "parent" ? "로그인" : tab === "student" ? "학습 시작하기" : "관리자 화면 열기"}<ArrowRight size={17} /></Button>
      </form>
      {demo ? <button onClick={() => router.push(tab === "parent" ? "/dashboard" : tab === "student" ? "/student" : "/admin/ai")} className="mt-5 flex w-full items-center justify-center gap-2 text-xs font-bold text-slate-500 hover:text-violet-700"><CheckCircle2 size={14} /> 데모 계정으로 바로 둘러보기</button> : null}
