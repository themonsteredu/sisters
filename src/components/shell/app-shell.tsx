      ...
      <div className="lg:pl-64">
        <header className="sticky top-0 z-30 flex h-16 items-center justify-between border-b border-[#dfe3e8] bg-[#f8f8f6]/95 px-4 backdrop-blur md:px-8 lg:h-20">
          ...
          <div className="flex items-center gap-2">
            <Link href={role === "student" ? "/notifications?role=student" : "/notifications"} className="relative grid size-10 place-items-center rounded-xl border border-[#d9dee5] bg-[#fafaf7] text-slate-600 hover:bg-[#f1f2f0]" aria-label={unread > 0 ? `알림 ${unread}건` : "알림"}><Bell size={19} />{unread > 0 ? <span className="absolute right-2 top-2 size-2 rounded-full bg-rose-500 ring-2 ring-[#fafaf7]" /> : null}</Link>
